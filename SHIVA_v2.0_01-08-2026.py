...
class SHIVA:
    ...
    def check_ppir(self):
        ...
        # Doom phrases that indicate fear-mongering (not historical/educational content)
        doom_phrases = [
            'you will fail', 'no hope', 'catastrophe awaits',
            'darkness consumes', 'all is lost', 'fear the', 'beware the curse',
            'you must obey', 'there is no escape', 'submit or perish'
        ]

        # Context words that indicate educational/historical use (not fear-mongering)
        educational_context = [
            'historical', 'ancient', 'mythology', 'translation', 'scholarship',
            '536', 'volcanic', 'old norse', 'rök', 'runestone', 'not destruction',
            'transcendence', 'philosophy', 'teaching', 'cycle', 'creation'
        ]
        ...
        for file_path in self.project_dir.rglob('*'):
            if file_path.suffix in ['.html', '.md', '.py']:
                ...
                try:
                    ...
                    # Count doom phrases, but check if in educational context
                    file_doom = 0
                    for phrase in doom_phrases:
                        if phrase in content:
                            # Check if file has educational context markers
                            has_edu_context = any(ctx in content for ctx in educational_context)
                            if not has_edu_context:
                                file_doom += 1

                    doom_count += file_doom
